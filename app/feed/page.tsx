...
export default function PreviewFeedLivePage() {
  ...
  // Helper function to redeem pending invite from sessionStorage (for post-OAuth flow)
  const redeemPendingInvite = async (userId: string): Promise<boolean> => {
    const vipCode = sessionStorage.getItem('vip_code')
    const inviteToken = sessionStorage.getItem('invite_token')
    
    if (vipCode) {
      console.log('Found pending VIP code, redeeming...')
      try {
        ...
        // Determine tier and update profile
        ...
        console.log(`✅ VIP code ${vipCode} redeemed after OAuth - awarded ${tier === 'boozehound' ? 10 : tier === 'alpha' ? 3 : 0} invites`)
      ...
      console.log('Found pending friend invite token, redeeming...')
      try {
        ...
        if (redeemResult?.success) {
          ...
          // Friend invites start with 0 invites
          await supabase
            .from('profiles')
            .update({
              invites_remaining: 0,
  ...
  const loadUser = async () => {
    ...
    // Try to fetch existing profile (include is_approved for invite gate check)
    ...
    if (profileError) {
      ...
      if (profileError.code === 'PGRST116') {
        ...
        // New user - check if they have a pending invite from OAuth flow
        const inviteRedeemed = await redeemPendingInvite(user.id)
        
        if (inviteRedeemed) {
          // Invite was redeemed successfully! Continue to profile setup
          console.log('Invite redeemed for new OAuth user, continuing to profile setup')
          setProfile({ ...createdProfile, is_approved: true })
          setShowProfileSetup(true)
        } else {
          // No pending invite - show gate
        ...
        const { data: basicProfile } = await supabase
          .from('profiles')
          .select('id, username, display_name, avatar_url, is_approved')
          .eq('id', user.id)
          .single()
        
        if (basicProfile) {
          setProfile({ ...basicProfile, has_seen_onboarding: true })
          // Check if user needs invite code or profile setup
          if (!basicProfile.is_approved) {
            setShowInviteCodeGate(true)
          } else if (!basicProfile.username || isAutoGeneratedUsername(basicProfile.username)) {
            setShowProfileSetup(true)
          }
        } else {
      ...
      // Check onboarding flow in order: invite code → profile setup → onboarding video
      if (!profileData.is_approved) {
        // User not approved - check if they have a pending invite from OAuth flow
        const inviteRedeemed = await redeemPendingInvite(user.id)
        
        if (inviteRedeemed) {
          // Invite was redeemed successfully! Update local state and continue to profile setup
          console.log('Invite redeemed after OAuth, continuing to profile setup')
          setProfile({ ...profileData, is_approved: true })
          setShowProfileSetup(true)
        } else {
          // No pending invite or redemption failed - show gate
          setShowInviteCodeGate(true)
        }
      } else if (!profileData.username || isAutoGeneratedUsername(profileData.username)) {
        // User needs to set up their profile (username, display name, bio)
        setShowProfileSetup(true)
      } else if (!profileData.has_seen_onboarding) {
        // User has completed profile but hasn't seen onboarding video
      ...
      {showProfileSetup && profile && user?.id && (
        <ProfileSetup
          ...
          onComplete={(updatedProfile) => {
            ...
            // After profile setup, show onboarding video if not seen
