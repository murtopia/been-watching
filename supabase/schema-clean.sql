...
DROP FUNCTION IF EXISTS public.handle_rating_activity();
DROP FUNCTION IF EXISTS public.handle_watch_status_activity();
...
-- Ratings table (Dislike/Like/Love system)
...
DROP POLICY IF EXISTS "Users can view all watch statuses" ON public.watch_status;
DROP POLICY IF EXISTS "Users can insert own watch status" ON public.watch_status;
DROP POLICY IF EXISTS "Users can update own watch status" ON public.watch_status;
DROP POLICY IF EXISTS "Users can delete own watch status" ON public.watch_status;
...
DROP POLICY IF EXISTS "Likes are viewable by everyone" ON public.activity_likes;
